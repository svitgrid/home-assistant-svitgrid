...
def test_per_unit_id_grouped_separately():
    spec = _spec(
        [
            {"field": "a", "address": 843, "unitId": 100},
            {"field": "b", "address": 784, "unitId": 247},
        ],
        protocol="modbus_tcp",
    )
    ranges = plan_ranges(spec)
...
def test_fc04_grouped_separately_from_fc03():
    spec = _spec(
        [
            {"field": "a", "address": 10, "functionCode": "FC03"},
...
def test_read_solarman_assembles_raw_registers(monkeypatch):
    ...
    fake_instance.read_holding_registers.assert_called_once_with(register_addr=100, quantity=2)
...
def test_read_modbus_assembles_raw_registers(monkeypatch):
    ...
    fake_client.read_holding_registers.assert_called_once_with(784, count=2, device_id=247)
