...
def test_merge_hourly_sample_weighted_mean():
    h1 = {
        "sample_count": 2,
        "avgs": {"pvPower": 1000.0},
        "peaks": {"pvPower": 1500.0},
        "energy": {"dailyPvEnergy": 4.0},
    }
    h2 = {
        "sample_count": 6,
        "avgs": {"pvPower": 2000.0},
        "peaks": {"pvPower": 3000.0},
...
def test_store_rollup_aggregates_completed_hour(tmp_path):
    ...
    try:
        rows = conn.execute(
            "SELECT inverter_id, hour_start, sample_count, avgs, peaks, energy FROM readings_hourly"
        ).fetchall()
        assert len(rows) == 1
        import json
...
def test_aggregate_includes_tier2_daily_counters():
    ...
    rows = [
        {"payload": {"dailyBatteryChargeEnergy": 5.0, "dailyGeneratorEnergy": 2.0}},
...
def test_aggregate_includes_tier3_losses_and_loadfreq():
    ...
    rows = [
        {"payload": {"dailyLossesEnergy": 1.0, "loadFrequency": 50.0}},
...
def test_aggregate_includes_daily_generator_runtime():
    ...
    rows = [
        {"payload": {"dailyGeneratorRuntime": 1.5, "dailyGeneratorEnergy": 2.0}},
...
def test_prune_drops_old_raw_keeps_daily(tmp_path):
    store = ReadingStore(None, str(tmp_path / "readings.db"))
    store._append_sync(
        {"inverterId": "inv-1", "timestamp": "2026-05-01T10:00:00Z", "pvPower": 1.0}
    )  # >14d before "now"
    store._append_sync({"inverterId": "inv-1", "timestamp": "2026-06-24T10:00:00Z", "pvPower": 1.0})
    pruned = store._prune_sync(
        "2026-06-24T12:00:00Z", raw_retention_s=14 * 86400, hourly_retention_s=2 * 365 * 86400
    )
    assert pruned["raw"] == 1
