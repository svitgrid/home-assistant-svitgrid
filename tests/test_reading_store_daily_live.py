...
def test_history_range_live_sealed_prior_plus_today_live(tmp_path):
    ...
    result = store._history_range_live_sync(inv, "2026-06-24", "2026-06-26", now_iso=now_iso)
...
def test_history_range_live_excludes_today_when_range_ends_before_today(tmp_path):
    ...
    store._append_sync({"inverterId": inv, "timestamp": "2026-06-26T09:00:00Z", "pvPower": 900.0})
...
def test_history_range_live_shape_matches_history_range(tmp_path):
    ...
    inv = "inv-1"
    now_iso = "2026-06-26T10:00:00Z"
    store._append_sync({"inverterId": inv, "timestamp": "2026-06-26T09:00:00Z", "pvPower": 500.0})
...
def test_history_range_live_ignores_other_inverters(tmp_path):
    ...
    store._append_sync(
        {"inverterId": "inv-1", "timestamp": "2026-06-26T09:00:00Z", "pvPower": 100.0}
    )
    store._append_sync(
        {"inverterId": "inv-2", "timestamp": "2026-06-26T09:00:00Z", "pvPower": 999.0}
    )

    result = store._history_range_live_sync("inv-1", "2026-06-26", "2026-06-26", now_iso=now_iso)
    assert len(result) == 1
...
def test_history_range_live_today_absent_from_sealed_table(tmp_path):
    ...
    live = store._history_range_live_sync(inv, "2026-06-26", "2026-06-26", now_iso=now_iso)
    assert len(live) == 1
