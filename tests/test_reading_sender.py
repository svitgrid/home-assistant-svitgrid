...
async def test_sender_drains_promptly_on_append(tmp_path):
    ...
    sender_task = asyncio.create_task(
        run_sender_loop(
            hass=hass, store=store, api_client=client,
            api_key="k", cadence=cadence, tick_s=TICK_S,
        )
    )
...
async def test_sender_fallback_sleep_when_store_lacks_wait_for_data(tmp_path):
    ...
    class _MinimalStore:
        ...
        async def skip_aged(self, now_iso, cap_s): return 0
        async def get_sendable(self, now_iso, cap_s, limit): return []
        async def mark_sent(self, keys): pass
        async def mark_failed(self, keys, now_iso): pass
    ...
    sender_task = asyncio.create_task(
        run_sender_loop(
            hass=hass, store=store, api_client=client,
            api_key="k", cadence=Cadence(interval_s=10),
